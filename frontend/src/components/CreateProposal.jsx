...
import { parsePAS } from '../utils/formatters'
import { getContracts } from '../utils/contracts'

export default function CreateProposal({ signer, refresh, pushToast, setBusy }) {
  const [description, setDescription] = useState('')
  const [amountPAS, setAmountPAS] = useState('')

  const disabled = useMemo(
    () => !signer || !description.trim() || !amountPAS || Number(amountPAS) <= 0,
    [signer, description, amountPAS],
  )
  ...
  async function submit() {
    try {
      ...
      const tx = await write.dao.createProposal(description.trim(), parsePAS(amountPAS))
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 shadow-glow">
      ...
      <div className="mt-1 text-sm text-white/60">
        Propose a PAS funding amount from the treasury with a clear description.
      </div>
      ...
      <div className="mt-4 grid gap-3">
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder="e.g. Fund grant for team building open-source Polkadot tooling…"
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
          <div>
            <label className="text-xs uppercase tracking-wider text-white/55">Funding amount (PAS)</label>
            <input
              value={amountPAS}
              onChange={(e) => setAmountPAS(e.target.value)}
              placeholder="25"
