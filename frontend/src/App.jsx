...
export default function App() {
  ...
  const [tokenMeta, setTokenMeta] = useState({ symbol: 'PGT', decimals: 18 })
    ...
    return async () => {
      ...
      try {
        ...
        const activeProposals = items.filter(
          (x) => now < Number(x.deadline) * 1000 && !x.executed,
        ).length
      ...
      if (e?.code === 4902) {
        if (!NETWORK.rpcUrls?.length) {
          pushToast({
            ...
            message:
              'This testnet is not configured in your MetaMask and no RPC URL was provided. Please add the Polkadot Testnet network manually, then click Switch again.',
          })
          return
        }
        await window.ethereum.request({
  ...
  async function mintFaucet() {
    if (!canWrite) {
      pushToast({ type: 'error', title: 'Wallet not ready', message: 'Connect and switch to the correct network.' })
      return
    }
    try {
      setBusy(true)
      const write = getContracts(signer)
      pushToast({ type: 'info', title: `Minting 1500 ${tokenMeta.symbol}…` })
      const tx = await write.faucet.requestTokens()
      pushToast({ type: 'tx', title: 'Mint submitted', txHash: tx.hash })
      await tx.wait()
      pushToast({ type: 'success', title: `Minted ${tokenMeta.symbol}` })
      await refresh()
    } catch (e) {
      pushToast({ type: 'error', title: 'Faucet mint failed', message: e?.shortMessage || e?.message })
    } finally {
      setBusy(false)
    }
  }
        ...
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden rounded-xl border border-polkaPink/35 bg-gradient-to-r from-polkaPink/15 to-polkaPurple/10 px-3 py-2 text-sm font-semibold text-white/90 transition hover:opacity-95 disabled:opacity-60 sm:inline-flex"
              title="Mint 1500 PGT from faucet"
            >
              Faucet: Mint 1500 {tokenMeta.symbol}
            </button>
            <ConnectWallet
      ...
      <main>
        {loading ? (
          <div className="mx-auto w-full max-w-7xl px-4 py-10">
            <LoadingSpinner label="Booting PolkaGrant DAO…" />
          </div>
        ) : (
          <Dashboard
            provider={provider}
